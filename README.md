...
개발자 뉴스/최신정보를 **화제도 + (나중엔) 관심사**로 골라 매일 새벽 정해진 시각에
완성된 브리핑으로 보내주는 PWA. 풀스택 TypeScript · Hono · SQLite · SvelteKit(PWA).

> v1은 **콜드스타트** 설계 — 산문 프로필 없이 화제도로 시작하고, 읽기 기록 대시보드가
> 진짜 관심사를 드러낸다. 적합도 가중치 학습은 v2.
...
브라우저에서 5173 접속 → 패스코드로 로그인 → **"지금 브리핑 생성"** 으로 즉시 한 건 만들기.
...
## v2 (데이터 쌓인 뒤)
읽기 기록 → 장르별 선호 학습 → 랭킹에 적합도 가중치 부활 / 산문 프로필 환원 / 👍👎 명시 피드백.
